...
class Suggest:
    ...
    def pretty_print(self):
        ...
        question_key = []
        question_desc = []
        for _, title, pitch, runtime, year, notes in self.file_contents[
            1:
        ]:  # this freaked out as a comprehension for some reason
            title = title.strip()
            ...
            question_desc.append(
                f"{title} ({year}, {int(runtime[0])}h{runtime[1]}m) - {pitch}{notes}"
            )
